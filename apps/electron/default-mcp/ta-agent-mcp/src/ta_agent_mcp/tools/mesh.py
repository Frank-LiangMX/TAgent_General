...
CHECK_MESH_BUDGET_TOOL = Tool(
    ...
    description=(
        "Check if a mesh meets polygon budget requirements. "
        "Returns triangle count, budget status, and optimization suggestions."
    ),
    inputSchema={
        "type": "object",
        "properties": {
...
CHECK_FBX_INFO_TOOL = Tool(
    ...
    inputSchema={
        "type": "object",
        "properties": {
            "file_path": {"type": "string", "description": "Path to the FBX file"},
            "include_materials": {
                "type": "boolean",
...
async def check_mesh_budget(
    file_path: str, budget: Optional[int] = None, lod_level: Optional[int] = None
) -> dict:
    """
    检查网格多边形预算
        ...
        if isinstance(scene, trimesh.Scene):
            ...
            for name, geometry in scene.geometry.items():
                if hasattr(geometry, "triangles"):
                    ...
                    mesh_info.append({"name": name, "triangles": tris})
        else:
            # 单个网格
...
async def check_fbx_info(
    file_path: str, include_materials: bool = True, include_animations: bool = True
) -> dict:
    """
    提取 FBX 文件信息
        ...
        if isinstance(scene, trimesh.Scene):
            ...
            for name, geometry in scene.geometry.items():
                if hasattr(geometry, "vertices"):
                    ...
                    mesh_data = {"name": name, "vertices": verts, "triangles": tris}

                    # 边界框
                    if hasattr(geometry, "bounds"):
                        bounds = geometry.bounds
                        mesh_data["bounds"] = {"min": bounds[0].tolist(), "max": bounds[1].tolist()}
            ...
            if hasattr(scene, "geometry"):
                for name, geometry in scene.geometry.items():
                    if hasattr(geometry, "visual") and hasattr(geometry.visual, "material"):
                        ...
                        materials.append({"mesh": name, "material": str(type(mat).__name__)})
            result["materials"] = materials

        # 动画信息（trimesh 不直接支持动画，标记为需要专用工具）
        if include_animations:
            result["animations"] = {"note": "动画信息需要使用 Blender 或 FBX SDK 提取"}
