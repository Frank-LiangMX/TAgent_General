...
SAVE_ASSET_TOOL = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            ...
            "asset_id": {"type": "string", "description": "Existing asset ID to update (optional)"},
        },
...
async def save_asset_tool(
    ...
    try:
        ...
        return format_result(
            data={"asset_id": saved_id, "name": name, "type": type, "path": path},
...
async def update_asset_tool(asset_id: str, **kwargs) -> dict:
    """
    更新资产属性
    """
    # 检查资产存在
    ...
    try:
        ...
        if success:
            ...
            return format_result(data=updated, message=f"资产已更新: {asset_id}")
        else:
...
async def delete_asset_tool(asset_id: str) -> dict:
    ...
    try:
        ...
        if success:
            return format_result(
                data={"asset_id": asset_id}, message=f"资产已删除: {existing['name']}"
            )
        else:
...
async def get_asset_tool(asset_id: str) -> dict:
    ...
    return format_result(data=asset, message=f"资产: {asset['name']}")
...
async def list_assets_tool(
    ...
    try:
        assets = list_assets(
            type=type, project=project, review_status=review_status, limit=limit, offset=offset
        )

        total = count_assets(type=type, project=project, review_status=review_status)
...
async def get_db_status_tool() -> dict:
    ...
    try:
        ...
        return format_result(data=status, message="数据库状态")
    except Exception as e:
