...
export const CONTEXT_USAGE_DESCRIPTIONS: Record<string, string> = {
  ...
  'Tool definitions': '与「内置工具」相同，部分 SDK 版本在分项中使用此名称。',
  'MCP tools':
    '已注册 MCP 服务器的工具定义。工具多或 schema 大时会显著占用上下文；标记「延迟」的尚未加载。',
  'Custom agents': 'Task 等子 Agent 的类型说明与调用约束，供主 Agent 派发子任务时使用。',
  Agents: '与「子 Agent」相同，子 Agent 定义与元数据占用。',
  Rules: '项目或用户规则文件（如 CLAUDE.md、.cursor/rules）注入到 prompt 的规则类内容。',
  'Free space': '当前窗口内尚未被分类项占用的可用额度。注意：不含「自动压缩预留」，两者分开统计。',
  Skills: '工作区 Skills 目录下的技能配置与 frontmatter，Agent 可按需引用。',
  Memory: 'TAgent 记忆层（用户画像、项目事实、L0-L5 等）注入到 prompt 的内容。',
  Conversation: '本会话累计的对话体：用户输入、助手回复、工具调用与返回等（与 Messages 同类项）。',
  Messages: '本会话消息主体：用户输入、助手回复、工具调用参数与工具返回结果等动态增长部分。',
  Attachments: '随消息附加或引用的文件、图片等，编码进上下文后的 token 体积。',
  'Slash commands': '内置斜杠命令（如 /compact）相关的提示与帮助文本注入。',
  'Autocompact buffer':
  ...
  工具结果:
    '工具执行后的返回内容（读文件、命令输出、MCP 结果等），往往是对话中增长最快的部分之一。',
  附件明细: '本分类下计入的附件与多媒体内容 token（图片、文档等）。',
  附件: '本分类下计入的附件与多媒体内容 token（图片、文档等）。',
  延迟加载: '该 MCP 工具尚未加载到当前上下文，仅占位或未计入；展开连接后体积可能上升。',
  ...
  流式估算: '圆环加载完成前的临时数值，来自 API 流式 usage，可能与 SDK 分项总和不一致。',
  分项刷新中: 'SDK 正在重新计算分项明细，摘要百分比可先参考。',
  分项为SDK估算: '各分类 token 由 Claude Agent SDK 估算，与供应商账单可能略有偏差。',
  压缩上下文: '调用 SDK 压缩流程，用模型摘要较早对话以释放窗口空间；对话进行中时不可用。',
  客户端快速压缩: '不调 LLM，直接丢弃较早的 tool 块以腾空间；速度快，但不如完整压缩保留的信息多。',
