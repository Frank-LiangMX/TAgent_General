 ...
 * - 监听 tagent:clear-context 和 tagent:focus-input 自定义事件
 * - 卡片式容器样式
 */
...
export function ChatInput({ conversationId, streaming, pendingAttachments, onSetPendingAttachments, onSend, onStop, onClearContext }: ChatInputProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('tagent:clear-context', handler)
    return () => window.removeEventListener('tagent:clear-context', handler)
  }, [onClearContext])
  ...
  return (
    <div className="session-input-dock px-2.5 pb-1 md:px-[18px] md:pb-2.5" data-input-mode="chat">
        <div
          className={cn(
