...
export function buildNavGraph(desks: OfficeDesk[]): NavGraph {
...
function estimateExitCost(
  ...
  if (!segmentCrossesSeat(sidePt.x, sidePt.y, aisle.x, desk.seatY, ctx, seatCheck)) {
    cost += Math.hypot(sidePt.x - aisle.x, sidePt.y - desk.seatY)
  } else {
    cost += 800
  }
