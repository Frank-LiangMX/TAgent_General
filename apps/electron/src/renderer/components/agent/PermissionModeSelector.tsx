...
export function PermissionModeSelector({ sessionId }: PermissionModeSelectorProps): React.ReactElement | null {
  ...
  const cycleMode = React.useCallback(async () => {
    const currentIndex = TAGENT_PERMISSION_MODE_ORDER.indexOf(mode)
    const nextIndex = (currentIndex + 1) % TAGENT_PERMISSION_MODE_ORDER.length
    const nextMode = TAGENT_PERMISSION_MODE_ORDER[nextIndex]!
    const prevMode = mode
