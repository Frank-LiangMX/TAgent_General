...
export function GlobResultRenderer({
  result,
  isError,
}: GlobResultRendererProps): React.ReactElement {
  if (isError) {
    return (
    ...
    (text: string): React.ReactNode => {
      ...
      return (
        <div className="space-y-1">
          <div className="text-[11px] text-muted-foreground/60">{files.length} 个文件</div>
          <div className="rounded-md bg-muted/20 p-2 space-y-0.5">
            {visibleFiles.map((file, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 text-[12px] font-mono text-foreground/70 py-0.5"
              >
