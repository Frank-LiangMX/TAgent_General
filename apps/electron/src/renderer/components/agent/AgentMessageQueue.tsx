...
import { CornerDownLeft, GripVertical, Paperclip, Quote, Trash2, Undo2, Clock3 } from 'lucide-react'
...
export function AgentMessageQueue({
  ...
  const handleDragOver = (event: React.DragEvent<HTMLDivElement>, targetId: string): void => {
  ...
  return (
    <div
      className="session-glass-popover pointer-events-auto mx-3 mb-2 max-h-44 overflow-y-auto rounded-glass-popover px-2.5 py-2"
        ...
        {items.map((item) => {
          ...
          return (
            <div
              ...
              className={cn(
                'session-list-item-active relative flex items-center gap-2 px-2 py-1.5 transition-colors',
            ...
            >
                ...
                {item.quotedSelection && (
                  <div className="mb-0.5 flex min-w-0 items-center gap-1 text-[11px] leading-4 text-muted-foreground">
                    ...
                    <span className="truncate">
                      引用：
                      {item.quotedSelection.filePath.split(/[/\\]/).pop() ??
                        item.quotedSelection.filePath}
                    </span>
                  </div>
                )}
              ...
              <div className="flex shrink-0 items-center gap-0.5">
                ...
                <QueueIconButton label="撤回到输入框" onClick={() => onRecall(item.id)}>
                  <Undo2 className="size-3.5" />
                </QueueIconButton>
                <QueueIconButton label="删除" onClick={() => onRemove(item.id)} danger>
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Button
          ...
          disabled={disabled}
          className={cn(
            'size-[36px] rounded-full text-foreground/60 hover:text-foreground',
