...
const MATCH_SELECTOR = 'mark[data-tagent-find-match]'
const SHADOW_STYLE_ID = 'tagent-find-highlight-style'
...
function injectShadowStyle(root: ParentNode): void {
  ...
  style.textContent = `
    ...
    ${MATCH_SELECTOR}[data-tagent-find-active="true"] {
...
function shouldSkipTextNode(node: Text): boolean {
  ...
  if (parent.closest('[data-tagent-find-ignore]')) return true
...
function markMatchesInTextNode(node: Text, matcher: RegExp): HTMLElement[] {
  ...
  for (const item of matches) {
    ...
    mark.dataset.tagentFindMatch = 'true'
...
function applyHighlights(
  ...
  marks.forEach((mark, index) => {
    mark.dataset.tagentFindIndex = String(index)
  })
  return marks
}
...
function setActiveMatch(marks: HTMLElement[], activeIndex: number): void {
  marks.forEach((mark, index) => {
    if (index === activeIndex) {
      mark.dataset.tagentFindActive = 'true'
      applyMatchStyle(mark, true)
      mark.scrollIntoView({ block: 'center', inline: 'nearest' })
    } else {
      delete mark.dataset.tagentFindActive
      applyMatchStyle(mark, false)
    }
  })
}
