...
export function VoiceDictationApp(): React.ReactElement {
  ...
  const startRecording = React.useCallback(async () => {
    ...
    if (permission.status === 'denied') {
      setStatus('error')
      setMessage('麦克风权限已被系统阻止，请在系统设置中允许 TAgent 访问麦克风')
      return
    }
    ...
    >
      <div ref={panelRef} className="flex min-h-0 w-full flex-col overflow-hidden">
        ...
        <div className="min-h-0 px-2">
          <div className="overflow-hidden rounded-glass-popover bg-muted/45">
            <div
              ref={hintBarRef}
  ...
  if (error instanceof DOMException) {
    switch (error.name) {
      ...
      case 'PermissionDeniedError':
        return '麦克风权限被系统阻止，请在 Windows 设置 > 隐私和安全性 > 麦克风中允许 TAgent 访问'
      case 'NotFoundError':
