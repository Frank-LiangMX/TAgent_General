...
/** 语音输入输出方式 */
export type VoiceDictationOutputMode = 'auto' | 'clipboard' | 'tagent-input'
...
export interface VoiceDictationCommitResult {
  mode: 'tagent-input' | 'cursor' | 'clipboard'
  success: boolean
  message: string
}
...
export interface AppSettings {
  ...
  /** 飞书 Session 镜像设置：每个 TAgent Session 可创建一个仅包含用户与指定 Bot 的飞书群 */
  feishuSessionMirror?: FeishuSessionMirrorSettings
  /** 启动时自动清理临时文件（tagent-preview、tagent-installers），默认 true */
