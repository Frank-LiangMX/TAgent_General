 ...
 * - 会话索引：~/.tagent/agent-sessions.json（轻量元数据）
 * - 消息存储：~/.tagent/agent-sessions/{id}.jsonl（JSONL 格式，逐行追加）
 *
...
export async function forkAgentSession(input: ForkSessionInput): Promise<AgentSessionMeta> {
  ...
  //   但旧消息仍保留在 TAgent JSONL 中，其 session_id 指向旧的 SDK session。
  ...
  // .context/ 必须保留 — TAgent 约定 .context/note.md、todo.md、plan/ 等是会话上下文，
 ...
 *
 * SDK session JSONL（~/.tagent/sdk-config/projects/...）中的消息都带有 uuid，
 * 但 TAgent 自己构造的 user message 没有 uuid。此函数直接读取 SDK 的 JSONL
 * 来解析 rewindFiles 所需的 user message UUID。
 *
 * 对于 fork 会话：TAgent JSONL 中的 UUID 来自**源会话**（fork 时直接复制），
...
function findSdkSessionJsonl(sdkSessionId: string, _projectDir?: string): string | undefined {
  ...
  // （SDK 的目录命名规则与 TAgent 不完全一致，直接遍历最可靠）
  const projectsDir = join(sdkConfigDir, 'projects')
  if (existsSync(projectsDir)) {
