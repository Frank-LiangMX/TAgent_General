...
function buildAutomationSchemas(z: ZodModule['z']) {
  ...
  return {
    ...
    create: {
      ...
      scheduleType: scheduleType.describe('?????????interval ?????????????daily ?????????weekly ???????????monthly ???????????once ??????????????????'),
      intervalMinutes: z.number().int().min(1).optional().describe('??????????????????????scheduleType=interval ?????????????????????? 10-30 ?????????? 1440=?????10080=????'),
      timeOfDay: z.string().optional().describe('??/????/???????????????24 ??????? HH:MM'),
      dayOfWeek: z.number().int().min(0).max(6).optional().describe('????????????0=????????1=???????...??6=??????'),
      dayOfMonth: z.number().int().min(1).max(31).optional().describe('????????????1-31??scheduleType=monthly ??????'),
      scheduledAt: z.number().int().positive().optional().describe('??????????????????????????????????????????scheduleType=once ??????????????"??????????????????????????"???? N ????/??????????????????????'),
      maxRuns: z.number().int().min(1).optional().describe('????????????????????????????????????????????+?????????????????????????????????????=??????????????? scheduleType ???????? interval+maxRuns=3 ??"?????????????????????? 3 ????"'),
      active: z.boolean().optional().describe('??????????????????? true'),
      permissionMode: permissionMode.optional().describe('????????????????????? bypassPermissions????????????????? auto'),
      sessionMode: sessionMode.optional().describe('???????daily=??????????????????????????????????????????????????????reuse=??????????????????????????????????????token ?????????????'),
    },
    update: {
      id: z.string().optional().describe('????????? ID??????????????????????????????????????????????'),
      ...
      scheduledAt: z.number().int().positive().optional().describe('???????????????????????????????????????scheduleType=once ???????'),
