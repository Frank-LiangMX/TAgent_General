...
export interface ClaudeAgentQueryOptions extends AgentQueryInput {
  ...
  maxTurns?: number
  /** SDK 权限模式（TAgent 当前三种模式直接映射 SDK 原生模式） */
  sdkPermissionMode: TAgentPermissionMode
  /** 是否跳过权限检查 */
...
const FRIENDLY_ERROR_MESSAGES: Array<{ pattern: RegExp; message: string }> = [
  {
    ...
    message: '请检查是否选择了正确的 TAgent 供应渠道和模型',
  },
  {
    pattern: /validation error/i,
    message:
      'API 请求格式校验失败，请点击下方「查看诊断详情」获取服务端返回的具体错误信息，或开启新会话',
  },
]
