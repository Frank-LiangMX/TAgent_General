...
class FeishuBridge {
  ...
  stop(): void {
    ...
    // 重启后仍需用来给桌面 Session 镜像建群。完整重置请删除 ~/.tagent/feishu-metadata-{botId}.json。
    this.botOpenId = null
  ...
  /**
   * 为 TAgent 桌面端会话准备飞书镜像群。
   *
   * 该群只包含用户与当前 Bot。用户在群里继续发送消息时，会通过
   * source=session-mirror 的绑定回到同一个 TAgent session。
   */
  ...
  private async createNewSession(
    ...
    if (!workspaceId) {
      await this.sendMessage(chatId, '请先在 TAgent 设置中创建工作区。')
      return
    }
  ...
  private handleAgentPayload(sessionId: string, payload: AgentStreamPayload): void {
    ...
    if (payload.kind === 'tagent_event' && payload.event.type === 'title_updated') {
