...
describe('飞书 Session 镜像设置', () => {
  ...
  test('Given 旧版通知配置 When 读取设置 Then 归一化为关闭', () => {
    expect(
      normalizeFeishuSessionMirrorSettings({
        mode: 'completion',
        botId: enabledBot.id,
      } as unknown as Parameters<typeof normalizeFeishuSessionMirrorSettings>[0])
    ).toEqual({ mode: 'off' })
  })

  test('Given 多个 Bot When 实时同步指定一个 Bot Then 只选择该 Bot', () => {
    const bot = resolveSessionMirrorBot({ mode: 'stream', botId: enabledBot.id }, [
      disabledBot,
      enabledBot,
    ])
  ...
  test('Given 指定 Bot 未启用 When 解析同步 Bot Then 不创建镜像', () => {
    const bot = resolveSessionMirrorBot({ mode: 'stream', botId: disabledBot.id }, [
      disabledBot,
      enabledBot,
    ])
