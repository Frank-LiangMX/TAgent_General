...
describe('KanbanDispatcher', () => {
  ...
  describe('并发上限 maxConcurrent', () => {
    test('5 个 ready 任务 maxConcurrent=3 时同时只跑 3 个', async () => {
      const board = db.createBoard({ rootGoal: 'G', parentSessionId: 's1' })
      ...
      configureKanbanDispatcher({ runner: mockRunner, db, maxConcurrent: 3 })
      startKanbanDispatcher()
