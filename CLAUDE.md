...
- **SDK auto-memory 入侵发现 + 根治方案落地**：SDK 0.3.153 的 `autoMemoryEnabled: false` 是空壳选项（schema 有定义，逻辑未实现），LLM 仍按 SDK 内置 system prompt 主动写记忆文件到 SDK 目录，导致自研 L0-L5 系统被架空。**双防线根治**：(1) system prompt 加 `MEMORY_MANAGEMENT_RULES` 反向指令禁止 LLM 主动写记忆 .md 文件（agent-prompt-builder.ts）；(2) `autoMemoryDirectory` 重定向到 `/tmp/tagent-discarded-memory/` 废目录兜底（memory-layer-service.ts 的 `getDiscardedMemoryDir`）。**Nudge 系统降低门槛**：检测间隔从 5 轮改为 1 轮 + fact_repeat 阈值从 ≥2 降到 ≥1 + behavior_repeat 从 ≥3 降到 ≥2 + 放宽正则（加"我叫 X" / "我喜欢 X" / "我是 X" 等日常表达）。删除临时方案残留（agent_self/ 重定向 + listAgentSelfFiles IPC + AgentSelfRow UI 组件）
