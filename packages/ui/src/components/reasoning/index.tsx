 ...
 * Reasoning - 推理/思考折叠组件
 *
 * Collapsible 包装，streaming 时自动展开，流式结束后自动折叠。
 * 使用 CSS animate-pulse 替代 Shimmer/framer-motion。
 *
 * 需要外部传入 onOpenExternal 回调以处理外部链接打开。
 */
...
export const Reasoning = React.memo(function Reasoning({
  ...
  return (
    <ReasoningContext.Provider value={{ isStreaming, isOpen, setIsOpen, duration, onOpenExternal }}>
      <Collapsible
...
export const ReasoningContent = React.memo(
  function ReasoningContent({
    ...
    const { onOpenExternal } = useReasoning()
