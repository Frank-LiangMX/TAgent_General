...
| 2 | context 爆了会话直接终止,连 context 工具都用不了 | P0 | 中 | ✅ 短期止血已落地（a/b/c 全部完成，2026-07-05）；长期根治（context 预算管理 + 多级 fallback + ContextUsageBadge 不依赖活跃 query）待做 |
| 3 | resume sessionId 错乱导致消息再也发送不了 | P0 | 低 | ✅ 已修复（P0+P1+P2 全部落地，2026-07-07） |
| 4 | kscc 报错无法反映到 UI,TAgent 背锅 | P0 | 低(短期) / 中(长期) | ✅ 短期修复已落地（2 处改动，2026-07-05）；长期（STREAM_ERROR IPC payload 升级 + extractApiError 第 4 种模式 + 日志文件兜底）待做 |
