...
8. **AppShell 采用固定三平面 + 浮层契约**：环境光场（0）→ 导航 / 单一主内容 / 检查器三个并列平面（10）→ 临时浮层（80+）。rail 与 sidebar 共用一个导航外框并以结构分隔线区分；三个主平面禁止负 margin 与互相覆盖。主内容边缘高光使用自身伪元素，禁止恢复多空节点描边或在业务组件追加壳层覆盖。
...
- 主题换色不再误伤材质语言
- 双轨 CSS 可收敛，维护成本下降
- 高透玻璃拥有真实的背景采样与前后关系，不需要靠额外阴影伪造景深
